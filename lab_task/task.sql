...
-- ALL EXLUSIVE, поэтому во время добавления таблица будет не доступна, но в Pstgres 11+
...
-- NO KEY UPDATE, это делается, чтобы не блокировать несвзянные таблицы, так как мы не обновляем
-- ключи и обновляем только один столбец, а не все. В итоге мы делаем апдейт с минимальной
-- возможной блокировкой и это не так сильно влияет на работоспосбность системы.
...
--
-- b) Здесь мы сначала делаем несколько подзапросов перед основным запрсом.
...
-- так как нам нужные только названия делталей, а не их количество
WITH pop AS (SELECT *
             FROM details
...
-- 11)
-- a)
-- Не совсем понял условие, но прирно это значит, что общая стоимость это сумма всех
-- ремнотов для каждой машины
-- Сначала находим машину с максимальной стоимостью рпемонта, потом находим самую дорогую деталь
...
;
-- с) Сначала находим максимальное число, которое тербовалось для ремонта, потом находим детали
-- с таким количестовом использования, потом делаем джоин в таблицу деталей, чтобы вывести более подробную
-- информацию
...
-- d) Здесь мы использкем any, то есть берем только те заказы, которые есть в
-- в мнжестве, полученном из подзапроса и джойним парочку таблиц, чтобы отобразить ответ
...
-- Вообщем то запрос не очень эффективный, но свое дело делает как надо
...
-- Ну чтобы он нашеля нам нужно просто добавить новые ремонты в таблику
...
;
-- с) Здесь снова есть одна дедаль, котарая хранится в атп3, но нет ни одного гаража
...
-- максимальным количестом

    SELECT *
    FROM details
    WHERE max_number = (SELECT max(max_number) FROM details)
    ;
-- b) Сначала нахожим нужные ремонты full_repairs, а потом сумму коммисий, потом, их количество
-- после этого вычислем среднюю коммисию, так же мы это делаем все в decimal
...
-- 15)
-- a)
-- Делаем подзапрос, в котором групируем детали и маштны попарно из реионтов и
-- там же складываем количестов, а потом просто делаем джоины, чтобы сделать отображение
-- более понятным
...
--b) Тут просто групируем и считаем, я честоно уже устал писать
...
-- нужно юзать групировку, то для всех наверное отдельно?
-- Тут просто делаем группировку по гараже айди и еще берем номер гаража, для большей наглядности
-- ну и получаем сумму для каждого гаража, так как во всех были какие-то ремонты чухих машинок
